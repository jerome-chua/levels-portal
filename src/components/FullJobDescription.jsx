...
export default function FullJobDescription({
  title, companyName, link, years, description, requirement, min, max, jobSkills, createdAt, saveJob, status, jobSaved,
}) {
  return (
        ...
        {status === 'USER'
          ? (
            <button
              id="save-job"
              type="submit"
              className={jobSaved ? 'ml-2 btn job-saved' : 'ml-2 btn btn-light'}
              onClick={saveJob}
            >
